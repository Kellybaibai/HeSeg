...
from torch_geometric.datasets import CitationFull, DeezerEurope
...
class GCNConv(nn.Module):
    def __init__(self, A, in_channels, out_channels, device):
        ...
        self.A_hat = A + torch.eye(A.size(0)).to(device)    #添加自环
    ...
    def set_A(self, A):
        ...
        # self.D = torch.diag(torch.sum(self.A_hat, 1))
        # self.D = self.D.inverse().sqrt()
        self.A_hat = torch.mm(torch.mm(self.D, A), self.D)
...
def get_adj(edge_index, num_nodes):
    ...
    for i in range(edge_index.shape[1]):
        src_node_idx = edge_index[0, i]
        tgt_node_idx = edge_index[1, i]
        adjacency_matrix[src_node_idx, tgt_node_idx] = 1
        adjacency_matrix[tgt_node_idx, src_node_idx] = 1  # For undirected graph
    return torch.tensor(adjacency_matrix)

class Data_model_preparation:
    def __init__(self, device):
        self.device = device
        # self.dataset = CitationFull('./pyg_data', 'DBLP')
        self.dataset = DeezerEurope('./pyg_data')
        print('dataset',dir(self.dataset))
        print(self.dataset.data)
        print(self.dataset.num_node_features)
        # print(self.dataset.data)
        self.avg_edges_per_node = self.dataset[0].edge_index.shape[1] / self.dataset[0].x.shape[0]
        print(f'Average edges per node: {self.avg_edges_per_node:.2f}')
        # self.pca_reduction()  # 是否做降维

    def get_graph_info(self):
        print(self.dataset.x[0].shape[0], self.dataset[0].edge_index.shape[1], self.dataset[0].x.shape[1])
        return self.dataset.x[0].shape[0], self.dataset[0].edge_index.shape[1], self.dataset[0].x.shape[1]

    def pca_reduction(self):
        # 标准化特征
        scaler = StandardScaler()
        # PCA降维
        features = self.dataset[0].x.cpu().numpy()   # 获取节点特征
        ...
        self.dataset[0].x = features_tensor


    def create_model(self,learning_rate,epoch_total):
        # 数据预处理和训练测试边划分
        u, v = self.dataset[0].edge_index[0], self.dataset[0].edge_index[1]
        print(u)
        print(v)
        eids = np.arange(self.dataset[0].edge_index.shape[1])
        ...
        neg_eids = np.random.choice(len(neg_u), self.dataset[0].edge_index.shape[1])
        test_neg_u, test_neg_v = neg_u[neg_eids[:num_test]], neg_v[neg_eids[:num_test]]

        # 先将邻接矩阵转换为稠密格式（如果是DGL图对象的话）
        A_dense = get_adj(self.dataset[0].edge_index, self.dataset[0].x.shape[0]).to_dense().to(self.device)
        # A_dense = self.g.adjacency_matrix().to_dense().to(self.device)
        # X = self.g.ndata['feat'].to(self.device)
        X = self.dataset[0].x.to(self.device)
        X_mean = X.mean(dim=0, keepdim=True)
        X_std = X.std(dim=0, keepdim=True) + 1e-9
        X_norm = (X - X_mean) / X_std

        # 然后创建模型实例
        model = GAE(A_dense, X.shape[1], hidden1=32, hidden2=20, device=self.device).to(self.device)
        ...
        n_node = self.dataset[0].x.shape[0]
        return model, test_pos_v,test_pos_u, test_neg_v,test_neg_u, X_norm, A_dense, n_node
...
class Evaluation:
    ...
    def sample_eval(self, q_epsilon):
        ...
        self.model.set_A(A_new.to(self.device))
...
def search_q(config, args):
    ...
    dataset_name = ['Myket','DBLP','DeezerEurope']
