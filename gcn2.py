...
from dgl.data import PubmedGraphDataset,CoraFullDataset, AmazonCoBuyComputerDataset, CoauthorCSDataset, RomanEmpireDataset,CoauthorPhysicsDataset, AmazonRatingsDataset, MinesweeperDataset, QuestionsDataset
import numpy as np
...
def get_args():
    ...
    parser.add_argument('--epsilon_min', type=float, default=0.25)
    parser.add_argument('--epsilon_max', type=float, default=1.4)
...
class Data_model_preparation:
    def __init__(self, device):
        ...
        # self.dataset = CoraFullDataset()
        # self.dataset = AmazonCoBuyComputerDataset()
        ...
        # self.dataset = MinesweeperDataset()
...
class Evaluation:
    def __init__(self, model, test_pos_v, test_pos_u, test_neg_v, test_neg_u, X_norm, A_dense, n_node, device):
        self.model = model
...
def model_pipeline(config,q=0.7):
    AUC_o_list = []
    AUC_s_list = []
    for i in range(10, 110, 20):
        ...
        auc_s,_,_ = evaluator.sample_eval(q)
    ...
    sub = AUC_o-AUC_s
    result = {'AUC_o':AUC_o,'AUC_s':AUC_s,'sub':sub}
    return result
...
def search_q(config, args):
    ...
    for i in range(10, 110, 10):
        ...
        ad = 0
        while Q_list[-1] * (1.03 + ad)< q_min_max[0]:
            Q_list = np.append(Q_list, int(Q_list[-1] * (1.03 + ad)))
            ad += 0.004
    ...
    dataset_name = ['Pumbed','CoauthorCS','Flickr','CoraFull','AmazonComputer','CoauthorPhysics','RomanEmpire','AmazonRatings','Minesweeper','Questions']
    data_name = dataset_name[6]
    with open('./performance/he_seg_' + data_name + '.txt','w') as f:
        ...
        f.write('Origin_AUC,Origin_std'+'\n')
        f.write(str(origin_score_mean) + ',' + str(origin_score_std) + '\n')
        f.write('Q,sample_edges,sample_AUC,sample_std,p_value' + '\n')
...
def search_param():
    # 定义搜索空间
    search_space = {
        "learning_rate": tune.grid_search(np.concatenate((np.arange(0.002, 0.01, 0.001),np.arange(0.01,0.15,0.01)))),
